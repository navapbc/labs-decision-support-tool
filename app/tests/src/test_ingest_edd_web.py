...
@pytest.fixture
def sample_cards_json(sample_cards):
    return json.dumps(sample_cards)


@pytest.fixture
def edd_web_local_file(tmp_path, sample_cards_json):
    file_path = tmp_path / "edd_scrapings.json"
    file_path.write_text(sample_cards_json)
    return str(file_path)


@pytest.fixture
def edd_web_s3_file(mock_s3_bucket_resource, sample_cards_json):
    mock_s3_bucket_resource.put_object(Body=sample_cards_json, Key="edd_scrapings.json")
...
def test__ingest_edd(
    caplog, app_config, db_session, sample_cards, edd_web_local_file, edd_web_s3_file, file_location
):
