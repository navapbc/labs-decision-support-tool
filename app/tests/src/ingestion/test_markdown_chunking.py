...
def test_copy_subtree(tiny_tree):
    list_tree = copy_subtree(tiny_tree["L_7"])
    assert list_tree.first_child() == list_tree["L_7"]
...
def test_copy_one_node_subtree(tiny_tree):
    paragraph_tree = copy_subtree(tiny_tree["P_4"])
    p_node = paragraph_tree.first_child()
    assert len(p_node.children) == 0
    assert repr(p_node.data) == repr(tiny_tree["P_4"].data)


def test_remove_children(caplog, tiny_tree):
    list_node = copy_subtree(tiny_tree["L_7"]).first_child()
