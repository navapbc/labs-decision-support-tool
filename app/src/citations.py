...
def _get_context(chunks: list[Chunk]) -> list[ChunkWithSubsection]:
...
def get_context_for_prompt(chunks: Sequence[Chunk]) -> str:
    ...
    context_list = []
    for index, (chunk, subsection) in enumerate(context):
    
        context_text = "Citation: citation-" + str(index) + "\n"
        context_text += "Document name: " + chunk.document.name + "\n"
        context_text += "Content: " + subsection
