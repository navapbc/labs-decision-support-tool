...
def remap_citation_ids(
    ...
    to a list of ChunkWithSubsection, where the order of the list reflects the order of the citations in `response`.
    ...
    [ChunkWithSubsection(id="1",...), ChunkWithSubsection(id="2",...)] so that
    ...
    for citation_id in deduped_indices:
        ...
        if citation_id in citation_map:
            ...
            citations.append(factory.create_citation(citation.chunk, citation.subsection))
    return citations
...
def add_citation_links(response: str, remapped_citations: Sequence[ChunkWithSubsection]) -> str:
    global _footnote_id
    _footnote_id += 1
    footnote_list = []
    citation_map = {c.id: c for c in remapped_citations}
