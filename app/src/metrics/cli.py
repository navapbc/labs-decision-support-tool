...
def main():
    ...
    retrieval_func = lambda query, k: retrieve_with_scores(query, k, args.min_score)
    ...
    try:
        ...
        for filename in os.listdir(log_dir):
            if filename.startswith("metrics_"):
                ...
                with open(filepath) as f:
                    metrics = json.load(f)
                    if latest_timestamp is None or metrics["timestamp"] > latest_timestamp:
                        latest_timestamp = metrics["timestamp"]
        ...
        if latest_results:
            ...
            print("\nError Analysis:")
            print(f"  Failed retrievals: {latest_results['error_analysis']['failed_retrievals']}")
            print(f"  Average score for failed retrievals: {latest_results['error_analysis']['avg_score_failed']:.4f}")
            print(f"  Common failure datasets: {', '.join(latest_results['error_analysis']['common_failure_datasets'])}")
